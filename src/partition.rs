...
pub struct BlkidParttable<'a>(PhantomData<&'a ()>, libblkid_rs_sys::blkid_parttable);
...
impl<'a> BlkidParttable<'a> {
    ...
    pub fn get_id(&self) -> Result<String> {
        Ok(
            unsafe { CStr::from_ptr(errno_ptr!(libblkid_rs_sys::blkid_parttable_get_id(self.1))?) }
                .to_str()?
                .to_string(),
        )
    }
    ...
    pub fn get_offset(&self) -> Result<BlkidBytes> {
        Ok(BlkidBytes::new(errno_with_ret!(unsafe {
            libblkid_rs_sys::blkid_parttable_get_offset(self.1)
        })?))
    }

    /// Get the parent partition in the case of nested partition tables.
    pub fn get_parent(&self) -> Option<BlkidPartition<'a>> {
        option_ptr!(unsafe { libblkid_rs_sys::blkid_parttable_get_parent(self.1) })
            .map(|p| BlkidPartition(PhantomData, p))
    }
}

/// A handle for working with a probed partition.
pub struct BlkidPartition<'a>(PhantomData<&'a ()>, libblkid_rs_sys::blkid_partition);
...
impl<'a> BlkidPartition<'a> {
    ...
    pub fn get_table(&self) -> Result<BlkidParttable<'a>> {
        Ok(BlkidParttable(
            PhantomData,
            errno_ptr!(unsafe { libblkid_rs_sys::blkid_partition_get_table(self.1) })?,
        ))
    }
    ...
    pub fn get_name(&self) -> Result<Option<String>> {
        let char_ptr =
            match option_ptr!(unsafe { libblkid_rs_sys::blkid_partition_get_name(self.1) }) {
                Some(ptr) => ptr,
                None => return Ok(None),
            };
    ...
    pub fn get_uuid(&self) -> Result<Option<Uuid>> {
        let char_ptr =
            match option_ptr!(unsafe { libblkid_rs_sys::blkid_partition_get_uuid(self.1) }) {
                Some(ptr) => ptr,
                None => return Ok(None),
            };
    ...
    pub fn get_type_string(&self) -> Result<String> {
        Ok(unsafe {
            CStr::from_ptr(errno_ptr!(
                libblkid_rs_sys::blkid_partition_get_type_string(self.1)
            )?)
        }
        .to_str()?
        .to_string())
    ...
    pub fn get_flags(&self) -> libc::c_ulonglong {
        unsafe { libblkid_rs_sys::blkid_partition_get_flags(self.1) }
    }

    /// Check whether the given partition is logical.
    pub fn is_logical(&self) -> bool {
        (unsafe { libblkid_rs_sys::blkid_partition_is_logical(self.1) }) != 0
    }

    /// Check whether the given partition is an extended partition.
    pub fn is_extended(&self) -> bool {
        (unsafe { libblkid_rs_sys::blkid_partition_is_extended(self.1) }) != 0
    }

    /// Check whether the given partition is a primary partition.
    pub fn is_primary(&self) -> bool {
        (unsafe { libblkid_rs_sys::blkid_partition_is_primary(self.1) }) != 0
    }
}

/// A handle for traversing a list of partitions.
pub struct BlkidPartlist<'a>(PhantomData<&'a ()>, libblkid_rs_sys::blkid_partlist);

impl<'a> BlkidPartlist<'a> {
    pub(crate) fn new(partlist: libblkid_rs_sys::blkid_partlist) -> BlkidPartlist<'a> {
        BlkidPartlist(PhantomData, partlist)
    }

    /// Get the number of partitions in the list.
    pub fn number_of_partitions(&mut self) -> Result<libc::c_int> {
        errno_with_ret!(unsafe { libblkid_rs_sys::blkid_partlist_numof_partitions(self.1) })
    }
    ...
    pub fn get_table(&mut self) -> Result<BlkidParttable<'a>> {
        Ok(BlkidParttable(
            PhantomData,
            errno_ptr!(unsafe { libblkid_rs_sys::blkid_partlist_get_table(self.1) })?,
        ))
    }

    /// Get a partition at the given index of the list.
    pub fn get_partition(&mut self, index: libc::c_int) -> Result<BlkidPartition<'a>> {
        Ok(BlkidPartition(
            PhantomData,
            errno_ptr!(unsafe { libblkid_rs_sys::blkid_partlist_get_partition(self.1, index) })?,
        ))
    }

    /// Get a partition by the number listed in the partition table. Correctly
    /// handles "out-of-order" partition tables.
    pub fn get_partition_by_partno(&mut self, num: libc::c_int) -> Result<BlkidPartition<'a>> {
        Ok(BlkidPartition(
            PhantomData,
            errno_ptr!(unsafe {
                libblkid_rs_sys::blkid_partlist_get_partition_by_partno(self.1, num)
            })?,
        ))
    }
    ...
    pub fn get_partition_by_devno(&mut self, dev: &BlkidDevno) -> Result<BlkidPartition<'a>> {
        Ok(BlkidPartition(
            PhantomData,
            errno_ptr!(unsafe {
                libblkid_rs_sys::blkid_partlist_devno_to_partition(self.1, dev.as_dev_t())
            })?,
        ))
    }
}
